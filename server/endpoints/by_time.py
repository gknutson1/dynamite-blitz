...
@app.get("/boards/{board}/by-time", description="Get records for a board, most recent first.", responses=
{
...
async def leaderboard(
    ...
    with Cursor() as conn:
        ...
        try:
            ...
            if user:
                conn.execute(
                    "select score, name, color_r, color_g, color_b, time from boards join users where boards.user = users.id and uname = ? order by time desc limit ? offset ?",
                    (user, count, start))
            else:
                conn.execute(
